#!/usr/bin/env python3
"""
CI/CD Course — Hetzner + Cloudflare Provisioner
================================================

Provisions 1 VM per student on Hetzner Cloud, then on each VM:
  - creates a 'student' user with passwordless sudo
  - installs Git, Docker (+ Compose plugin), and Trivy
  - adds the student user to the docker group
...
Prerequisites:
    ...
    Copy .env from the project root and fill in credentials.
...
HETZNER_KEY_NAME = "cicd-course-provisioner"
FW_NAME          = "cicd-course-fw"

# slug   : used for the Hetzner VM name and DNS subdomain (ASCII, no umlauts)
# display: shown in the final summary
STUDENTS = [
    {"slug": "ben-coeppicus",        "display": "Ben Cöppicus"},
    {"slug": "yasin-acarbas",        "display": "Yasin Acarbas"},
    {"slug": "ernest-berezovskyy",   "display": "Ernest Berezovskyy"},
    {"slug": "manuel-berg",          "display": "Manuel Berg"},
    ...
    {"slug": "mirsad-vejseli",       "display": "Mirsad Vejseli"},
    {"slug": "antonius-reifferscheid", "display": "Antonius Reifferscheid"},
]
...
# ─────────────────────────────────────────────────────────────────────────────
# Phase 4 — Docker, Trivy, code-server, Caddy
# ─────────────────────────────────────────────────────────────────────────────
...
apt-get update -qq
NEEDRESTART_SUSPEND=1 DEBIAN_FRONTEND=noninteractive apt-get install -y -qq trivy
"""
...
apt-get update -qq
NEEDRESTART_SUSPEND=1 DEBIAN_FRONTEND=noninteractive apt-get install -y -qq caddy
"""


def configure_vm(ip: str, slug: str, password: str) -> None:
    """
    Install the full CI/CD toolstack on one VM:
      Git (already present from base), Docker + Compose, Trivy, code-server, Caddy.
    """
    ...
    with connection(ip) as c:
        log(f"  [{slug}] Installing Docker + Compose plugin ...")
        c.run(_DOCKER_INSTALL, hide=True)
        # Allow the student user to run docker without sudo
        c.run("usermod -aG docker student", hide=True)
...
def configure_all_vms(vm_ips: dict[str, str]) -> dict[str, str]:
    ...
    log("=== Phase 4: Docker + Trivy + code-server + Caddy ===")
...
def print_summary(vm_ips: dict[str, str], passwords: dict[str, str]) -> None:
    sep = "=" * 72
    print(f"\n{sep}")
    print("  PROVISIONING COMPLETE — CI/CD Course")
    print(sep)
    for s in STUDENTS:
        slug = s["slug"]
        ip   = vm_ips[slug]
    ...
    print("  • Docker Compose is available as 'docker compose' (v2 plugin).")
    print("  • The student user is in the docker group — no sudo needed for docker.")
    print("  • Trivy is available system-wide as 'trivy'.")
    print(sep)

    # Write a machine-readable credentials file alongside this script
    md_path = Path(__file__).parent / "cicd-course-credentials.md"
    rows = "\n".join(
        f"| {s['display']} "
    ...
    md_path.write_text(
        "# CI/CD Course — Credentials\n\n"
        "| Student | URL | Password | IP |\n"
        "|---|---|---|---|\n"
        f"{rows}\n\n"
        "SSH login: `ssh student@<ip>`  \n"
        "Docker Compose: `docker compose` (v2 plugin, no sudo required)  \n"
        "Trivy: `trivy image <image>` / `trivy fs .`\n"
    )
